...
\begin{frame}{Beispiel}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.7\textwidth}
			...
			\only<6->{
				\textbf{Latenzen}:
				\begin{itemize}
					\item<6-> $L(\text{\textcolor{blue}{blau}}) = f(a)^2 + f(b)$
					\item<6-> $L(\text{\textcolor{green}{grün}}) = f(b)$
					\item<6-> $L(\text{\textcolor{red}{rot}}) = f(a)^2$
				\end{itemize} }
		\end{column}
	\end{columns}
	...
	\only<7->{
		\textbf{Kosten}: \\
		 $C(f)$ = $0.1 \cdot L(\text{blau}) + 0.7 \cdot L(\text{grün}) + 0.2 \cdot L(\text{rot})$ }
\end{frame}
...
\begin{frame}{Nashfluss im Beispiel}
	...
	\begin{columns}[T]
		\begin{column}{0.4\textwidth}
			Nashfluss $f$:
			\begin{itemize}
				\item $f(blau) = \frac{-1 + \sqrt{5}}{2} $
				\item $f(\text{grün}) = \frac{3 - \sqrt{5}}{2} $
				\item $f(rot) = 0 $
			\end{itemize}
		\end{column}
...
\begin{frame}{Equilibrium im Beispiel}
	...
	\begin{figure}
		...
		\end{tikzpicture}
	\end{figure}
	Unabhängig von $f$ die MBR von ist $g$.
\end{frame}
