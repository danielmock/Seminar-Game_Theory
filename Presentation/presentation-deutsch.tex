...
\begin{frame}{Beispiel}
	...
	\begin{figure}
		...
		% Fluss
		\draw [blue, ultra thick, ->, visible on=<2->, opacity=0.5] plot [smooth] coordinates{(s) (P1) (Q1) (P2) (Q2) (t)};
		\draw [green, ultra thick, ->, visible on=<3->, opacity=0.5] plot [smooth] coordinates{(s) (P2) (Q2) (t)};
		\draw [red, ultra thick, ->, visible on=<4->, opacity=0.5] plot [smooth] coordinates{(s) (P1) (Q1) (t)};
	\end{tikzpicture}
	\end{figure}
	...
	\begin{columns}[T]
		\begin{column}{0.3\textwidth}
			\only<1-5>{
				\textbf{Flusswerte}:
				\begin{itemize}
					\item<2-> $f(\text{\textcolor{blue}{blau}}) = 0.1$
					\item<3-> $f(\text{\textcolor{green}{grün}}) = 0.7$
					\item<4-> $f(\text{\textcolor{red}{rot}}) = 0.2$
				\end{itemize} }
		...
		\begin{column}{0.7\textwidth}
			...
			\only<6->{
				\textbf{Latenzen}:
				\begin{itemize}
					\item<6-> $L(\text{\textcolor{blue}{blau}}) = f(a)^2 + f(b) = 0.89$
					\item<6-> $L(\text{\textcolor{green}{grün}}) = f(b) = 0.8$
					\item<6-> $L(\text{\textcolor{red}{rot}}) = f(a)^2 = 0.09$
				\end{itemize} }
		\end{column}
	\end{columns}
...
\begin{frame}{Nashfluss im Beispiel}
	\begin{figure}
		\begin{tikzpicture}
			...
			% Fluss
			\draw [red, ultra thick, ->] plot [smooth] coordinates{(s) (P1) (Q1)  (t)};
...
\begin{figure}
	...
	% Fluss
	\draw [red, ultra thick, ->, visible on=<2->] plot [smooth] coordinates{(s) (P1) (Q1) (t)};
	\end{tikzpicture}
\end{figure}
\end{frame}
...
\begin{frame}{Equilibrium im Beispiel}
	...
	\begin{figure}
		\begin{tikzpicture}
			...
			% Fluss
			\draw [red, ultra thick, ->, visible on=<1->] plot [smooth] coordinates{(s) (P1) (Q1) (P2) (Q2) (t)};
		\end{tikzpicture}
	\end{figure}
...
\begin{frame}{Equilibrium im Beispiel}
	...
	\begin{figure}
		...
		% Fluss
		\draw [green, ultra thick, ->] plot [smooth] coordinates{(s) (P1) (Q1)  (t)};
		\draw [green, ultra thick, ->] plot [smooth] coordinates{(s) (P2) (Q2) (t)};
		\end{tikzpicture}
	\end{figure}
...
\begin{frame}{Braess' Paradoxon}
	\begin{columns}[T]
		...
		\begin{column}{0.5\textwidth}
			...
			% Nashfluss
			\SetUpEdge[style={->, thick}, color=green]
			\tikzset{LabelStyle/.style = {above, sloped, fill=none, color=green}}
			\Edge[label=$1$](s)(p)
			\Edge[label=$0.5$](p)(m)
			\Edge[label=$0.5$](p)(n)
			\tikzset{LabelStyle/.style = {below, sloped, fill=none, color=green}}
...
%			\Edge(n)(q)
			...
			\Edges(p,m,q)
			\Edges(p,n,q)
			\SetUpEdge[style={->, ultra thick}, color=green]
			\Edges(s,q,t)
			\Edges(s,p,t)
			\end{tikzpicture}
